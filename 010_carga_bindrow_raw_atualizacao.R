...
sql <- paste0("SELECT * FROM `noticias-423520.noticias.noticias_anpd` WHERE data_formatada >= '", data_maxima_anpd,"'")
# Executar a query
query_job <- bq_project_query("noticias-423520", sql)
# Obter os resultados da query
df_raw_anpd <- bq_table_download(query_job)

sql <- paste0("SELECT * FROM `noticias-423520.noticias.noticias_google_news` WHERE published_date >= '", data_maxima_google_news,"'")
query_job <- bq_project_query("noticias-423520", sql)
...
if(nrow(df_tratada)>0){
  ...
  bigrquery::dbWriteTable(con, tablename, df_tratada, append=T, verbose = T, row.names=F, overwrite=F, fields=df_tratada)
