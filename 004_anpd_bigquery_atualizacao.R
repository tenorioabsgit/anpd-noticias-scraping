...
if(nrow(df_final)>0){
  ...
  bigrquery::dbWriteTable(con, tablename, df_final, append=T, verbose = T, row.names=F, overwrite=F, fields=df_final)
  print("Dataframe Ocorrencias Uploaded")
}

sql <- ("CREATE OR REPLACE TABLE
`noticias-423520.noticias.noticias_anpd` AS 
SELECT DISTINCT *
FROM 
    `noticias-423520.noticias.noticias_anpd`
ORDER BY
    data_formatada DESC
")
